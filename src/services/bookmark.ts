...
export const createBookmark = async ({
  ...
  try {
    ...
    if (created.status === Status.ERROR || Number(created.code) > 226)
...
export const fetchBookmarks = async (
  ...
  try {
    ...
    if (bookmarks.status === Status.ERROR || Number(bookmarks.code) > 226)
      return internalResponse(
        true,
        Number(422),
...
export const fetchABookMark = async (authToken: string, id: string) => {
  try {
    ...
    if (aBookmark.status === Status.ERROR || Number(aBookmark.code) > 226)
      return internalResponse(
        true,
        Number(422),
...
export const deleteABookmark = async (authToken: string, id: string) => {
  try {
    ...
    if (deleted.status === Status.ERROR || Number(deleted.code) > 226)
      return internalResponse(
        true,
        Number(422),
